...
class CategoryService:
    ...
    async def edit_category(self, category_edit: CategoryEdit, category_id: int, user: User) -> Category:
        return await self.repository.edit_category(await self.get_category_by_id(category_id, user), category_edit)
    ...
    async def get_category_by_id(self, category_id: int, user: User) -> Category:
        ...
        if category is None or category.id == user.base_category_id:
    ...
    async def delete_category(self, category_id: int, user: User) -> None:
        return await self.repository.delete_category(await self.get_category_by_id(category_id, user))
